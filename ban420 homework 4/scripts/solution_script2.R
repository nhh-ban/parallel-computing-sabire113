...
cores <- 8  # Set the number of CPU cores you want to use
cl <- makeCluster(cores)
registerDoParallel(cl)

# Modified loop using parallel processing
df <- expand.grid(
...
df <- foreach(i = 1:nrow(df), .packages = c("tweedie", "magrittr", "tidyverse"), .combine = rbind) %dopar% {
  print(i)
  share_reject <- MTests(df$N[i], df$M[i], df$type[i], 0.05)
  data.frame(N = df$N[i], M = df$M[i], type = df$type[i], share_reject = share_reject)
}
